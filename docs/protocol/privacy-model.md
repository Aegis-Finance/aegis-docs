...
| Your wallet as gas payer, if a relayer submits | Tx `from` — the relayer if you used one; otherwise you |

A Sonic transaction always has a gas payer. **Privacy entry** exists so that payer does not have to be you: you sign an EIP-712 intent; a relayer calls `PrivacyEntryRouter` and pays `S`. The explorer still shows *a* `from` — that is the relayer’s address, not your wallet. If you submit the intent yourself, you remain the gas payer.
...
When public entry is off, you use **Privacy entry** with EIP-712 intents — optionally relayed so someone else pays gas. A relayer hides **who paid gas**, not the on-chain outcome of the call.
